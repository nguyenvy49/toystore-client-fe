...
const SearchForm = ({ onSearch }: { onSearch: (keyword: string) => void }) => {
  const [keyword, setKeyword] = useState("");

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const lowerKeyword = keyword.trim().toLowerCase(); // 🔹 Chuyển về chữ thường
    onSearch(lowerKeyword); // Gửi về component cha
  };
